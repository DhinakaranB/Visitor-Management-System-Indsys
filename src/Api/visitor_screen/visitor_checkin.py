...
def handle_search(search_term, root_instance):
    ...
    if not response or "data" not in response:
        ui_elements["status_lbl"].config(text="NOT FOUND", fg=ERROR_COLOR)
        return
...
def perform_checkout():
    ...
    if response and response.get("code") == "0":
        ...
        ui_elements["status_lbl"].config(text="CHECKED OUT", fg=ERROR_COLOR)
...
def show_checkin_screen(root_instance, show_main_screen_callback):
    ...
    # --- Main Container ---
    container = tk.Frame(root_instance, bg=BG_COLOR, padx=40, pady=20)
    ...
    tk.Label(header_frame, text="  |  Visitor Check-Out", font=("Segoe UI", 24, "bold"), bg=BG_COLOR, fg=TEXT_COLOR).pack(side="left")

    # --- Search Card ---
    search_card = tk.Frame(container, bg=CARD_BG, padx=30, pady=20)
    search_card.pack(fill="x", pady=(0, 20))

    tk.Label(search_card, text="SEARCH VISITOR ID", font=("Segoe UI", 8, "bold"), bg=CARD_BG, fg=LABEL_COLOR).pack(anchor="w")
    ...
    # --- Details Card ---
    details_card = tk.Frame(container, bg=CARD_BG, padx=40, pady=40)
    ...
    add_detail_row(4, "GENDER", "purpose_lbl", None, None) 
